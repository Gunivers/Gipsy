...
class Groups(commands.Cog):
    ...
    async def group_join(self, ctx: MyContext, group: GroupConverter):
        ...
        # if group is private and user is not an admin
            ...
            await ctx.author.add_roles(group, reason="Joined a group")
    ...
    async def group_leave(self, ctx: MyContext, group: GroupConverter):
        ...
        if not group.member_is_in(ctx.author):
            ...
            return
        await ctx.author.remove_roles(group, reason="Left a group")
    ...
    async def group_admin_add(self, ctx: MyContext, group: GroupConverter, user: discord.Member):
        ...
        await ctx.send(await self.bot._(ctx.guild.id, "groups.joinbyforce", name=group, user=user.name))
    ...
    async def group_admin_remove(self, ctx: MyContext, group: GroupConverter, user: discord.Member):
        ...
        if not group.member_is_in(ctx.author):
            ...
            return
        await user.remove_roles(group)
        await ctx.send(await self.bot._(ctx.guild.id, "groups.leavebyforce", name=group, user=user.name))
